...
Private char* opname_strings[] = {"CAT", "LT",  "GT", "EQ",    "NE",     "LE",
                                  "GE",  "AND", "OR", "MATCH", "MATCHES"};
Private int16_t opname_codes[] = {TKN_COLON, TKN_LTX,     TKN_GTX,    TKN_EQ,
 ...
 */
  ...
  bool copy_char;
  int16_t j;
  int32_t alloc_size = 32; /* Size to request from s_alloc() */
  int16_t actual_size;    /* Actual size allocated */
      ...
      while ((bytes_remaining-- > 0) && !done_after) {
          ...
          switch (token_type) {
            case TKN_NAME:
              if (c == '_') {
                ...
                if (bytes_remaining)
                  x = *p; /* 0344 */
                else if (str_hdr->next != NULL)
                  x = str_hdr->next->data[0];
                else
                  x = '\0';
                ...
                if (bytes <= LONGEST_OPNAME) /* Could be an operator name */
                {
                  ...
                  *tgt = '\0';
                  for (j = 0; j < NUM_OPCODE_NAMES; j++) {
                    if (stricmp(tgt_str->data, opname_strings[j]) ==
                        0) /* 0475 */
                    {
            ...
            case TKN_LABEL:
              if (c == ':')
                done_after = TRUE;
              else if (strchr(label_chars, c) == NULL)
                done = TRUE;
              break;
        ...
        if (copy_char) {
          if (tgt_bytes_remaining == 0) {
            new_tgt = s_alloc((int32_t)alloc_size, &actual_size);
          ...
          if ((token_type == TKN_NAME) && !case_sensitive)
            c = UpperCase(c);
          *(tgt++) = c;
          tgt_bytes_remaining--;
          tgt_str->bytes++;
