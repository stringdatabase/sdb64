 ...
 */
  ...
  int32_t field;           /* Start field for search */
  int32_t value;           /* Start value for search */
  int32_t subvalue;        /* Start subvalue for search */
  int32_t index;           /* Index position to return (working) */
  int32_t result;          /* Index position to return (actual) */
  ...
  int16_t search_bytes_remaining;
  int32_t search_string_len;
  char order_string[2 + 1]; /* Ordering code */
  int16_t order_string_len;
  int16_t status = 0; /* Assume failed for now */
  bool ordered = FALSE;
...
#define MAX_NUMERIC_STRING 20
  ...
  int16_t numeric_len;
  int32_t search_value;        /* Search item as a number */
  ...
  if (order_string_len < 0)
    goto exit_locate; /* Failed to extract ordering string */
    ...
    switch (UpperCase(order_string[1])) {
      case 'R':
        ...
        if (numeric_mode) {
          ...
          if (search_hdr != NULL) {
            if ((n = (int16_t)(search_hdr->string_len)) <=
                MAX_NUMERIC_STRING) {
              ...
              numeric_len = (int16_t)(search_hdr->string_len);
              numeric_compare =
                  strnint(numeric_string, numeric_len, &search_value);
            }
          }
        }
        break;
  ...
  if (numeric_compare) {
    ...
    while (1)     /* For each source chunk */
    {
      while (src_bytes_remaining-- > 0) {
        ...
        if (IsDelim(c) && ((u_char)c >= (u_char)mark)) {
          if (skipping) {
            ...
            if (strnint(item_string, item_len, &item_value)) /* Is numeric */
            {
              ...
              if ((item_value < search_value) != ascending)
                goto found;
            } else /* Not numeric */
            {
              if (item_len == numeric_len) {
                if ((SortCompare(item_string, numeric_string, item_len,
                                 nocase) < 0) != ascending)
                  goto found;
              } else if ((item_len < numeric_len) != ascending)
                goto found;
            }
          }

          if (c != mark)
            goto not_found; /* Higher mark */
        ...
        } else if (!skipping) /* Still in item */
        {
          ...
          {
            if (ascending)
              goto not_found; /* Gone past desired item */
    ...
    if (!skipping) {
      ...
      {
        if (item_len == numeric_len) {
          if ((SortCompare(item_string, numeric_string, item_len, nocase) <
               0) != ascending)
            goto not_found;
        } else if ((item_len < numeric_len) != ascending)
          goto not_found;
  ...
  while (1) {
    while (src_bytes_remaining-- > 0) {
      ...
      } else /* Still in item */
      {
        ...
        if (nocase)
          x = UpperCase((int16_t)c) - UpperCase(*(srch++));
        else
          x = ((int16_t)c) - *(srch++);
        ...
        if (x != 0) /* Found a difference */
        {
          if (ordered) {
            if (right) {
              ...
              src_len =
                  search_offset + search_hdr->bytes - search_bytes_remaining;
              ...
              do {
                while (src_bytes_remaining > 0) {
                  ...
                  if (IsDelim(c)) {
                    ...
                    {
                      if (src_len == search_string_len)
                        goto compare;
                      if ((search_string_len < src_len) == ascending)
                        goto found;
                      goto skip_item;
                    }
                  }
  ...
  if ((search_offset == 0) &&
      ((search_hdr == NULL) || (search_bytes_remaining == search_hdr->bytes))) {
