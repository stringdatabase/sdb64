...
char *argv[];
{
  ...
  emit(
      "[ SDFIX %s   Copyright, Ladybridge Systems, %s.  All rights reserved. "
      "]\n\n",
...
/* rev 0.9.0 */
  ...
  if (!file_found)
    printf("No SD hashed files found matching given name(s)\n");

exit_sdfix:
  return status;

usage:
  printf("Usage: SDFIX {options} pathname(s)\n\n");
  printf("Options:\n");
  printf("   -F      Fix errors unconditionally\n");
  printf("   -L      Log output to sdfix.log\n");
...
#define CHK_ECB 3
  ...
  /* Correct the longest_id field. This is a silent fix as it is likely
    ...
    length limit of the SD system we are using.                         */
...
bool open_dump_file() {
  ...
  sprintf(dump_file_name, "~SDFix.%d", (int32_t)getuid()); /* changed to %d from %lu */
