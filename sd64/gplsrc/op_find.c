 ...
 */
  ...
  int16_t srch_bytes_remaining;
  char* srch;

  DESCRIPTOR* descr;
  int32_t field;    /* Current field... */
  int32_t value;    /* ...value... */
  int32_t subvalue; /* ...and subvalue */
  ...
  if (src_hdr != NULL) {
    ...
    do {
      ...
      while (src_bytes_remaining--) {
        c = *(src++);
        if (IsDelim(c)) {
          if (!skipping && (srch_hdr == NULL) && (--occurrence == 0))
            goto found;
          skipping = FALSE;
          switch (c) {
            case FIELD_MARK:
    ...
    if (skipping || (srch_hdr != NULL) || --occurrence)
      status = 0; /* Not found */
