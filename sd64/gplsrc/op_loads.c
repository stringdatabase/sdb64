...
void op_clrcom() {
  ...
  #define block_namesz 4   /* 3 char + \0 */
  char block_name[block_namesz];

  /* Find blank common */
  /*  20240122  mab change sprintf to snprintf and test for buffer overflow */
  if (snprintf(block_name, block_namesz, "$%d", (int)cproc_level) >=  block_namesz){
     k_error("Overflowed block_name length in op_clrcom().");
     return;
  }
