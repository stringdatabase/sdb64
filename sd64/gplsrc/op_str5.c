 ...
 */
  ...
  ((e_stack++)->data.value) = (int32_t)checksum;
}
 ...
 */
  ...
  if (str1_len > str2_len)
    d = 1; /* Can't do d = str1_len - str2_len... */
  else if (str1_len < str2_len)
    d = -1; /* ...as lengths are long ints. */
 ...
 */
  ...
  int32_t src_f, src_v, src_s;
  int32_t tgt_f, tgt_v, tgt_s;
  char c;
 ...
 */
  ...
  int16_t src_bytes_remaining; /* Bytes left to process in current chunk */

  char delimiter; /* Substring delimiter character */

  int32_t occurrence; /* Substring occurrence to replace */

  int32_t no_of_substrings; /* Number of substrings to replace */

  /* Replacement string */
  STRING_CHUNK* rep_str;         /* Current chunk */
  char* rep;                     /* Character pointer into chunk */
  int16_t rep_bytes_remaining; /* Bytes left to process in current chunk */

  DESCRIPTOR tgt_descr; /* Target string */

  bool insert_all = FALSE; /* No of substrings negative? */
  int32_t skip_count;     /* Number of substrings to skip */
  bool nocase;
  int16_t n;
  char s[2];
  char* p;
  ...
  } else /* Insert no_of_substrings components of replacement substring */
  {
    if (rep_str != NULL) {
      while (no_of_substrings) {
        ...
        } else /* Delimiter found */
        {
          n = p - rep;
          if (--no_of_substrings)
            n++; /* Copy delimiter except in final substring */
        }
    ...
    if (--no_of_substrings > 0)
      ts_fill(delimiter, no_of_substrings);
  }
 ...
 */
  ...
  while (list_str != NULL) {
    ...
    while (list_bytes--) {
      ...
      {
        if (ci == delim)
          goto exit_op_listindx; /* Found it */
 ...
 */
  ...
  int32_t value; /* Value to extract */
  int32_t v;     /* Current value position */
  bool want_this_value;
  int32_t marks_pending = 0;
 ...
 */
  ...
  int16_t from_len;       /* ...and length */
  u_char to_str[256 + 1];   /* Translation string... */
  int16_t to_len;         /* ...and length */
