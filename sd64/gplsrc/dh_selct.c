...
Private u_int32_t upd_ct[HIGH_SELECT + 1];

/* ======================================================================
   Start select on given file                                             */

bool dh_select(DH_FILE* dh_file, /* File descriptor */
               int16_t list_no) {
  FILE_ENTRY* fptr;
  ...
  if (select_file[list_no] != NULL) /* Clear down partial select */
  {
    ...
    while (fptr->file_lock < 0)
      Sleep(1000); /* Clear file in progress */
    (fptr->inhibit_count)--;
  }
      ...
      do {
        ...
        while (rec_offset < used_bytes) {
          ...
          /* Maintain the record counts. These two variables are not
              ...
              standard Q_M command do something of this sort when they
...
void dh_end_select_file(DH_FILE* dh_file) {
  ...
  if (fptr->inhibit_count >
      0) /* There's a select (etc) somewhere - Is it us? */
  {
...
bool dh_select_group(DH_FILE* dh_file, /* File descriptor */
                     int16_t list_no) {
  bool status = FALSE;
  int32_t group;
  int16_t group_bytes;
