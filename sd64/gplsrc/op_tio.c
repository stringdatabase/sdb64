  ...
  if (at1 >= 0) {
    if (at1 < tio.dsp.width) {
      if ((at2 >= 0) && (at2 < tio.dsp.lines_per_page)) {
        p = tparm(&n, sdtgetstr("cup"), at2, at1);
      } else /* Column address only */
      {
        /* Not all terminals have the hpa capability. If this is not
          ...
          code so that we do not have to do repeated sdtgetstr() searches on
    ...
    switch (at1) {
      ...
      case IT_IL: /* @(-17)  -  Insert line */
        p = (at2 < 1) ? sdtgetstr("il1") : tparm(&n, sdtgetstr("il"), at2);
        break;

      case IT_DL: /* @(-18)  -  Delete line */
        p = (at2 < 1) ? sdtgetstr("dl1") : tparm(&n, sdtgetstr("dl"), at2);
        break;

      case IT_ICH: /* @(-19)  -  Insert character */
        if (at2 < 1)
          at2 = 1;
        emitn(s, sdtgetstr("ich1"), at2);
        p = s;
        break;
      ...
      case IT_CUF: /* @(-34, n)  -  Cursor forward */
        ...
        if (at2 > tio.dsp.width)
          at2 = tio.dsp.width;
        emitn(s, sdtgetstr("cuf1"), at2);
        p = s;
        break;

      case IT_FGC: /* @(-37)  -  Set foreground colour */
        p = tparm(&n, sdtgetstr("setf"), colour_map(at2));
        break;

      case IT_BGC: /* @(-38)  -  Set background colour */
        p = tparm(&n, sdtgetstr("setb"), colour_map(at2));
        break;
      ...
      case IT_ACMD: /* @(-108)  -  Execute asynchronous command */
        p = tparm(&n, sdtgetstr("u8"), arg2_str);
        break;

      case IT_SCMD: /* @(-109)  -  Execute synchronous command */
        p = tparm(&n, sdtgetstr("u9"), arg2_str);
        break;
...
Private int colour_map(int user_colour) {
  ...
  if (tio.terminfo_colour_map == NULL) {
    tio.terminfo_colour_map = sdtgetstr("colourmap");
    if (tio.terminfo_colour_map == NULL)
 ...
 */
  ...
  switch (key) /* !!PU!! */
  {
    ...
    case PU_BANNER:
      setsdstring(&(pu->banner), descr);
      break;

    case PU_FORM:
      setsdstring(&(pu->form), descr);
      break;
    ...
    case PU_OPTIONS:
      setsdstring(&(pu->options), descr);
      break;

    case PU_PREFIX:
      setsdstring(&(pu->prefix), descr);
      break;

    case PU_SPOOLER:
      setsdstring(&(pu->spooler), descr);
      break;

    case PU_OVERLAY:
      setsdstring(&(pu->overlay), descr);
      break;
...
/* 20240219 mab rebrand VBSRVR to APISRVR */
  if ((connection_type == CN_NONE) || is_sdApiSrvr)
    phantom_error();
...
/* 20240219 mab rebrand VBSRVR to APISRVR */
  if (!is_phantom && !is_sdApiSrvr) /* 0542 */
  {
    p = sdtgetstr("is1");
    n = strlen(p);
...
/* 20240219 mab rebrand VBSRVR to APISRVR */
    if (is_sdApiSrvr)
      return TRUE; /* Ignore output from SDServer */
...
/* 20240219 mab rebrand VBSRVR to APISRVR */
  if (!capturing && !is_sdApiSrvr) /* 0139 */
  {
    p = sdtgetstr("clear");
    n = strlen(p);
...
/* ======================================================================
   op_readpkt()  - Read SDClient data packet                              */
...
/* ======================================================================
   op_writepkt()  -  Write SDClient data packet                           */
...
/* 20240219 mab rebrand VBSRVR to APISRVR */    
    case CN_SOCKET:
      if (is_sdApiSrvr) /* Do not do TN parameter mangling */
      {
